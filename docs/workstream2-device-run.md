...
1. ロボホンに「**領域展開**」/「**領域展開して**」と話しかける（home.hvmlの起動ワード `WORD_APPLICATION` / `WORD_APPLICATION_FREEWORD`。詳細は末尾「起動ワード（現行）」）。
2. アプリが起動し、`onResume` の `startSpeech` で greet（「**いくよ……領域展開！**」）を発話すればゲート達成。※初回起動時は先に外部送信の同意ダイアログが背中の画面に出るので、選択してから会話が始まる。
...
## 起動ワード（現行）
固定文言「**領域展開**」。ロボホンの名前とは無関係（旧方式のロボホン名起動は、本体標準の呼びかけ応答と競合して起動しづらいため廃止した）。
- 標準: 「**領域展開**」（`${Local:WORD_APPLICATION} eq りょういきてんかい`）
- フリーワード: 「**領域展開して**」（`${Local:WORD_APPLICATION_FREEWORD} eq りょういきてんかいして`）
- 定義: `app/src/main/assets/hvml/home/com_robohon_template_home.hvml`
- 起動後の口上: greet で「いくよ……領域展開！」→ そのまま聞き取り待受へ
