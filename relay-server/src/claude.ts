...
function buildSongDanceBlock(catalog?: Catalog): string {
  ...
  return (
    ...
    `・例外: 誕生日の歌（ハッピーバースデー等）は、たとえ上の一覧に有っても perform_motion では歌わない。` +
    `一覧の曲は宛名を差し替えられず、いつも同じ人の名前で歌ってしまうため。誕生日の歌は必ず sing_birthday を使い、` +
    `誰にうたうか分からなければ先に「だれにうたう？」と聞き返す。`
  );
}
...
export const TOOLS: Anthropic.Messages.ToolUnion[] = [
  ...
  },
  {
    name: "sing_birthday",
    description:
      "「◯◯にハッピーバースデー歌って」「◯◯の誕生日をお祝いして歌って」のように、" +
      "誕生日の歌を“相手の名前入りで”歌ってほしいと頼まれたときに呼ぶ。name には歌に差し込む呼び名だけを入れる（例:「たろう」「おかあさん」）。" +
      "**誰の誕生日か分からないときは、このツールを絶対に呼ばず、テキストだけで「だれにうたう？」と聞き返す（聞き返しとツール呼び出しを同じターンで両方やらない）。名前が分かった次のターンで呼ぶ。**" +
      "name に「君」「あなた」「みんな」のような仮の呼び名を入れてはいけない（実際にその言葉で歌ってしまうため）。実在の呼び名が分かっているときだけ呼ぶ。" +
      "誕生日の歌は必ずこのツールを使う（perform_motion(kind=sing) の純正曲は宛名を差し替えられず、いつも同じ人の名前で歌ってしまうため使わない）。" +
      "歌の前に短い前置き（例『うたうね！』）をテキストで返してよい。",
    input_schema: {
      type: "object",
      properties: {
        name: { type: "string", description: "歌に差し込む相手の呼び名（短く。敬称や説明は付けない）。" },
      },
      required: ["name"],
      additionalProperties: false,
    },
