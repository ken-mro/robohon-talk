...
- `prompts/digest.md` … `/digest`（ナレッジベース日次更新）用の指示プロンプトの正データ。
- `src/digest.ts` … `/digest` エンドポイント。端末の「既存KB＋新規会話ログ」から Sonnet 4.6 で更新版KBを生成して返す（サーバは保存しない）。KBは `/chat` の `knowledge` としてペルソナに注入される。
